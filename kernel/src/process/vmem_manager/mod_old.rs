...
use crate::alloc::{zm, PageAllocator};
use crate::mem::{Allocation, PageLayout, PhysFrame, VirtFrame};
...
impl PageTable
{
	...
	unsafe fn free_if_empty<'a, T: PageAllocator + 'a>(&mut self, allocer: &'a T) -> bool
	{
		if self.count() == 0 {
	...
	unsafe fn dealloc<'a, T: PageAllocator + 'a>(&mut self, allocer: &'a T)
	{
		let frame = Allocation::new(self.addr(), PAGE_SIZE);
		allocer.dealloc_frame(frame);
	}

	unsafe fn dealloc_all<'a, T: PageAllocator + 'a>(&mut self, allocer: &'a T)
	{
		self.dealloc_recurse(allocer, 3);
	}

	unsafe fn dealloc_recurse<'a, T: PageAllocator + 'a>(&mut self, allocer: &'a T, level: usize)
	{
		if level > 0 {
	...
	fn get_or_alloc<'a, 'b, T: PageAllocator + 'a>(
		&'a mut self,
		index: usize,
		allocer: &'b T,
		flags: PageTableFlags,
	...
	unsafe fn remove<T: PageAllocator>(&mut self, index: usize, allocer: &T) -> bool
	{
...
impl VirtLayoutElement
{
	...
	pub fn from_range(phys_range: APhysRange, flags: PageMappingFlags) -> Self
	{
	...
	fn as_phys_zone(&self) -> Option<APhysRange>
	{
		...
		match self.phys_data {
			...
			VirtLayoutElementType::AllocedMem(mem) => {
				Some(APhysRange::new(virt_to_phys(mem.addr()), self.map_size))
			},
			VirtLayoutElementType::Mem(mem) => Some(APhysRange::new(mem.addr(), self.map_size)),
...
{
	virt_map: IMutex<BTreeMap<VirtRange, VirtLayout>>,
	cr3: IMutex<PageTablePointer>,
...
impl<T: FrameAllocator> VirtMapper<T>
{
	...
	pub fn new(frame_allocer: &'static T) -> VirtMapper<T>
	{
		...
		VirtMapper {
			virt_map: IMutex::new(BTreeMap::new()),
			cr3: IMutex::new(pml4_table),
	...
	fn contains(
		btree: &mut IMutexGuard<BTreeMap<VirtRange, VirtLayout>>,
		virt_zone: VirtRange,
	) -> bool
	{
	...
	fn find_range(
		btree: &IMutexGuard<BTreeMap<VirtRange, VirtLayout>>,
		size: usize,
	) -> Option<VirtRange>
	{
...
impl<T: FrameAllocator> Drop for VirtMapper<T>
{
	...
	// FIXME: this should not be in drop
	fn drop(&mut self)
	{
		unsafe {
			self.cr3
