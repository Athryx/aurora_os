...
/// A pointer to an x86 global descriptor table in the format the cpu needs for the lgdt instruction
#[repr(C, packed)]
...
/// 
/// This is used to store the current kernel rsp value
/// It also stores the value for all of the ist stacks
#[repr(C, packed)]
#[derive(Debug, Clone, Copy)]
pub struct Tss {
	zero1: u32,
...
/// An entry in the global descriptor table which refers to the tss
#[repr(C, packed)]
#[derive(Debug, Clone, Copy)]
struct TssEntry {
	limit1: u16,
...
impl TssEntry {
    ...
    // TODO: figure out how pin works and use it here
...
pub fn init() {
	cpu_local_data().gdt.lock().load(&cpu_local_data().tss.lock());
